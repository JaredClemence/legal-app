...
abstract class BaseServiceProvider {
    ...
    abstract public function save(\stdClass $item);
    ...
    protected function getAuthorizationHeader(){
        $zone = $this->zone;
        if(!isset($this->auths[$zone])){
            $this->loadAuth($zone);
        }
        $auth = $this->auths[$zone];
        $type = $auth->type;
        $token = $auth->token;
    ...
    protected function loadAuth(){
        ...
        list( $client_id, $secret ) = $this->getCredentialsForNickname( $zone );
